...
export default function Hero() {
  ...
  const handleStartGrinding = () => {
    if (isAuthenticated) {
      navigate('/problems'); // User is logged in, send 'em to problems!
    } else {
      navigate('/login'); // Not logged in? Gotta sign in first, fam.
    }
  };
  // --- END NEW LOGIC ---
  ...
  return (
    <section 
      ...
      className="relative min-h-[80vh] flex flex-col items-center justify-start text-center px-6 sm:px-12 pt-32 pb-20 bg-gradient-to-b from-white to-slate-50 overflow-hidden"
    >
      {/* KEPT: Your original blobs */}
      ...
      <div className="z-10 w-full max-w-5xl">
        ...
        <div className="flex items-center justify-center gap-4 mt-8">
          ...
          {/* IMPROVISED: Added onClick */}
          <button 
            onClick={handleStartGrinding}
            className="px-8 py-3 rounded-lg bg-gradient-to-br from-slate-900 to-slate-700 text-white text-sm sm:text-base font-bold flex items-center gap-3 shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300"
          >
          ...
          <button 
            ...
            className="px-6 py-3 rounded-lg border border-gray-300 bg-white/50 backdrop-blur-sm text-sm sm:text-base font-bold text-gray-700 flex items-center gap-3 hover:border-slate-400 transition"
          >
